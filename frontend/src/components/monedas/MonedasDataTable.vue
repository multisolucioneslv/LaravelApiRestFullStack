  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ monedas.length }} de {{ total }} monedas
        </span>
      </div>
...
const columns = [
  ...
  },
  {
    accessorKey: 'codigo',
    header: 'Código',
    cell: ({ row }) => h('div', { class: 'font-medium uppercase' }, row.getValue('codigo')),
    enableHiding: false,
  },
  {
    accessorKey: 'nombre',
  ...
  },
  {
    ...
    id: 'simbolo',
    header: 'Símbolo',
    cell: ({ row }) => h('div', { class: 'font-semibold' }, row.getValue('simbolo')),
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const tasa = parseFloat(row.getValue('tasa_cambio'))
      return h('div', { class: 'font-mono' }, tasa.toFixed(4))
    },
    enableHiding: true, // Toggleable
  },
  {
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(moneda => moneda.id)
  emit('bulkDelete', ids)
