<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between gap-4">
      <Input
        v-model="searchQuery"
        placeholder="Buscar por ruta, controlador, método..."
        class="max-w-sm"
        @input="handleSearch"
      />
...
const columns = [
  ...
  },
  {
    accessorKey: 'id',
    header: 'ID',
    cell: ({ row }) => h('div', { class: 'font-medium w-12' }, row.getValue('id')),
    enableHiding: false,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'font-medium' }, sistema?.nombre || '-')
    },
    enableHiding: true,
  },
  {
    ...
    id: 'ruta',
    header: 'Ruta',
    cell: ({ row }) => h('div', { class: 'font-mono text-sm max-w-xs truncate' }, row.getValue('ruta')),
    enableHiding: false,
  },
  {
    accessorKey: 'metodo',
  ...
  },
  {
    ...
    id: 'descripcion',
    header: 'Descripción',
    cell: ({ row }) => h('div', { class: 'max-w-md truncate text-sm' }, row.getValue('descripcion') || '-'),
    enableHiding: true,
  },
  {
    ...
    header: 'Controlador',
    cell: ({ row }) => {
      const controlador = row.getValue('controlador')
      const accion = row.original.accion
      const displayText = accion ? `${controlador}@${accion}` : controlador
      return h('div', { class: 'font-mono text-xs max-w-xs truncate', title: displayText }, displayText || '-')
    },
    enableHiding: true,
  },
  {
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(ruta => ruta.id)
  emit('bulkDelete', ids)
