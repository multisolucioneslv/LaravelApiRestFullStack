  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ galerias.length }} de {{ total }} galerías
        </span>
      </div>
...
const columnLabels = {
  ...
  cantidad_imagenes: 'Cant. Imágenes',
}
...
const columns = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const galeria = row.original
      const imagenUrl = galeria.imagen_principal
      ...
      return imagenUrl
        ...
        : h('div', {
            class: 'w-12 h-12 rounded bg-gradient-to-br from-gray-300 to-gray-400 dark:from-gray-600 dark:to-gray-700 flex items-center justify-center text-white font-semibold'
          }, '?')
    },
    enableHiding: true,
  },
  {
    accessorKey: 'cantidad_imagenes',
    id: 'cantidad_imagenes',
    header: 'Cant. Imágenes',
    cell: ({ row }) => {
      const cantidad = row.getValue('cantidad_imagenes')
      return h('div', {
        class: 'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400'
      }, cantidad)
    },
    enableHiding: true,
  },
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex gap-2' }, [
        ...
        h(Button, {
          variant: 'ghost',
          size: 'sm',
          onClick: () => emit('delete', galeria.id),
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(galeria => galeria.id)
  emit('bulkDelete', ids)
