<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between gap-4">
      <Input
        v-model="searchQuery"
        placeholder="Buscar por sexo o inicial..."
        class="max-w-sm"
        @input="handleSearch"
      />
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ sexes.length }} de {{ total }} sexos
        </span>
      </div>
...
const columns = [
  ...
  },
  {
    accessorKey: 'sexo',
    header: 'Sexo',
    cell: ({ row }) => h('div', { class: 'font-medium' }, row.getValue('sexo')),
    enableHiding: false,
  },
  {
    accessorKey: 'inicial',
    header: 'Inicial',
    cell: ({ row }) => h('div', { class: 'font-semibold text-center uppercase' }, row.getValue('inicial')),
    enableHiding: false,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'text-sm text-gray-600 dark:text-gray-400' }, formatted)
    },
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(sex => sex.id)
  emit('bulkDelete', ids)
