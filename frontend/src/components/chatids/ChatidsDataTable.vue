<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between gap-4">
      <Input
        v-model="searchQuery"
        placeholder="Buscar por ID de Telegram..."
        class="max-w-sm"
        @input="handleSearch"
      />
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ chatids.length }} de {{ total }} chat IDs
        </span>
      </div>
...
const columns = [
  ...
  {
    accessorKey: 'idtelegram',
    header: 'ID de Telegram',
    cell: ({ row }) => h('div', { class: 'font-medium' }, row.getValue('idtelegram')),
    enableHiding: false,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', {}, date.toLocaleDateString('es-ES', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
        hour: '2-digit',
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex gap-2' }, [
        ...
        h(Button, {
          ...
          onClick: () => emit('delete', chatid.id),
          title: 'Eliminar chat ID'
        }, () => h('svg', {
          xmlns: 'http://www.w3.org/2000/svg',
          width: '18',
          height: '18',
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(chatid => chatid.id)
  emit('bulkDelete', ids)
