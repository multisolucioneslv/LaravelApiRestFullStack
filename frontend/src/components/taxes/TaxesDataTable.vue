  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ taxes.length }} de {{ total }} impuestos
        </span>
      </div>
...
const columns = [
  ...
  },
  {
    ...
    header: 'Porcentaje',
    cell: ({ row }) => {
      const porcentaje = row.getValue('porcentaje')
      return h('div', { class: 'font-medium text-blue-600 dark:text-blue-400' }, `${porcentaje}%`)
    },
    enableHiding: false,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const empresaNombre = tax.empresa?.nombre || '-'
      return h('div', { class: 'text-sm' }, empresaNombre)
    },
    enableHiding: false,
  },
  {
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(tax => tax.id)
  emit('bulkDelete', ids)
