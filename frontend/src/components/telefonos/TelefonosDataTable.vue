  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ telefonos.length }} de {{ total }} teléfonos
        </span>
      </div>
...
const columns = [
  ...
  },
  {
    ...
    header: 'Teléfono',
    cell: ({ row }) => h('div', { class: 'font-medium' }, row.getValue('telefono')),
    enableHiding: false,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex gap-2' }, [
        ...
        h(Button, {
          ...
          onClick: () => emit('delete', telefono.id),
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(telefono => telefono.id)
  emit('bulkDelete', ids)
