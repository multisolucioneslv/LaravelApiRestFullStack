  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ inventarios.length }} de {{ total }} inventarios
        </span>
      </div>
...
const columns = [
  ...
  },
  {
    ...
    id: 'codigo',
    header: 'C�digo',
    cell: ({ row }) => h('div', { class: 'font-mono text-sm' }, row.getValue('codigo')),
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'max-w-xs truncate' }, desc || '-')
    },
    enableHiding: true, // Toggleable
  },
  {
    ...
    header: 'Bodega',
    cell: ({ row }) => {
      const bodega = row.original.bodega
      return h('div', {}, bodega?.nombre || '-')
    },
    enableHiding: true, // Toggleable
  },
  {
  ...
  },
  {
    ...
    id: 'cantidad',
    header: 'Cantidad',
    cell: ({ row }) => h('div', { class: 'text-center' }, row.getValue('cantidad')),
    enableHiding: true, // Toggleable
  },
  {
    accessorKey: 'minimo',
    id: 'minimo',
    header: 'M�nimo',
    cell: ({ row }) => h('div', { class: 'text-center' }, row.getValue('minimo')),
    enableHiding: true, // Toggleable
  },
  {
    accessorKey: 'maximo',
    id: 'maximo',
    header: 'M�ximo',
    cell: ({ row }) => h('div', { class: 'text-center' }, row.getValue('maximo')),
    enableHiding: true, // Toggleable
  },
  {
    ...
    cell: ({ row }) => {
      const precio = row.getValue('precio_compra')
      return h('div', { class: 'text-right' }, `$${parseFloat(precio).toFixed(2)}`)
    },
    enableHiding: true, // Toggleable
  },
  {
  ...
  },
  {
    ...
    header: 'Galer�a',
    cell: ({ row }) => {
      const galeria = row.original.galeria
      return h('div', {}, galeria?.nombre || '-')
    },
    enableHiding: true, // Toggleable
  },
  {
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(inventario => inventario.id)
  emit('bulkDelete', ids)
  rowSelection.value = {} // Limpiar selecci�n
}
