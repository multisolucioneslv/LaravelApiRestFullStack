  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        <span v-else>
          Mostrando {{ settings.length }} de {{ total }} configuraciones
        </span>
      </div>
...
const columns = [
  ...
  },
  {
    ...
    id: 'clave',
    header: 'Clave',
    cell: ({ row }) => h('div', { class: 'font-mono text-sm' }, row.getValue('clave') || '-'),
...
const handleBulkDelete = () => {
  const ids = selectedRows.value.map(setting => setting.id)
  emit('bulkDelete', ids)
