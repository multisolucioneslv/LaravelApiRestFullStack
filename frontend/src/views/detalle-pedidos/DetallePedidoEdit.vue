      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600 dark:text-gray-400 mt-2">
            Modifica los datos del detalle de pedido
          </p>
        </div>
        ...
        <CardContent class="pt-6">
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="space-y-2">
              ...
              <Select v-model="form.pedido_id" required>
                <SelectTrigger>
                  <SelectValue placeholder="Seleccione pedido" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem
                    ...
                    :value="pedido.id"
                  >
                    PED-{{ pedido.codigo || pedido.id }} - {{ pedido.cliente?.nombre || pedido.empresa?.nombre || 'N/A' }} ({{ pedido.fecha }})
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
...
const loadDetalle = async () => {
  try {
    const detalle = await fetchDetallePedido(detalleId)

    // Llenar el formulario con los datos existentes
    form.pedido_id = detalle.pedido_id
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await updateDetallePedido(detalleId, dataToSend)
    goToIndex()
  } catch (error) {
