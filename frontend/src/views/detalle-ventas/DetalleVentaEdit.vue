      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600 dark:text-gray-400 mt-2">
            Modifica los datos del detalle de venta
          </p>
        </div>
...
const loadDetalle = async () => {
  try {
    const detalle = await fetchDetalleVenta(detalleId)

    // Llenar el formulario con los datos existentes
    form.venta_id = detalle.venta_id
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await updateDetalleVenta(detalleId, dataToSend)
    goToIndex()
  } catch (error) {
