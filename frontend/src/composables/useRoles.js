...
export function useRoles() {
   ...
   */
  const fetchRoles = async (page = 1, showLoading = true) => {
    loading.value = true
    error.value = null

    if (showLoading && authStore.showLoadingEffect) {
      alert.loading('Cargando lista de roles', 'Por favor espere...')
    }
  ...
  const fetchRole = async (id) => {
    ...
    if (authStore.showLoadingEffect) {
      alert.loading('Cargando datos del rol', 'Por favor espere...')
    }
  ...
  const fetchAllPermissions = async () => {
    ...
    if (authStore.showLoadingEffect) {
      alert.loading('Cargando permisos', 'Por favor espere...')
    }
  ...
  const createRole = async (roleData) => {
    ...
    if (authStore.showLoadingEffect) {
      alert.loading('Creando rol', 'Por favor espere...')
    }
  ...
  const updateRole = async (id, roleData) => {
    ...
    if (authStore.showLoadingEffect) {
      alert.loading('Actualizando rol', 'Por favor espere...')
    }
  ...
  const deleteRole = async (id) => {
    ...
    if (authStore.showLoadingEffect) {
      alert.loading('Eliminando rol', 'Por favor espere...')
    }
    ...
    try {
      ...
      // Recargar lista de roles (sin mostrar loading adicional)
      await fetchRoles(currentPage.value, false)
  ...
  const searchRoles = async (searchTerm) => {
    ...
    await fetchRoles(1, true) // Reiniciar a la página 1 y mostrar loading
  }
